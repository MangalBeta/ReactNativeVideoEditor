...
import { useIsForeground } from '../CameraHooks';
...
try{
          ...
          <View style={styles.rightButtonRow}>
            {supportsCameraFlipping && (
              <PressableOpacity style={styles.button} onPress={onFlipCameraPressed} disabledOpacity={0.4}>
                 <CustomText color="white">{'camera-reverse'}</CustomText>
              </PressableOpacity>
            )}
            {supportsFlash && (
              <PressableOpacity style={styles.button} onPress={onFlashPressed} disabledOpacity={0.4}>
                                <CustomText color="white">{flash === 'on' ? 'flash' : 'flash-off'}</CustomText>
            ...
            {supportsHdr && (
              <PressableOpacity style={styles.button} onPress={() => setEnableHdr((h) => !h)}>
                  ...
                  <CustomText color="white">{enableHdr ? 'hdr' : 'hdr-off'}</CustomText>
              </PressableOpacity>
            )}
            {canToggleNightMode && (
              <PressableOpacity style={styles.button} onPress={() => setEnableNightMode(!enableNightMode)} disabledOpacity={0.4}>
                                <CustomText color="white">{enableNightMode ? 'moon' : 'moon-outline'}</CustomText>
    ...
    console.log(err,"erorororoorororooror")
    return null
}
}
